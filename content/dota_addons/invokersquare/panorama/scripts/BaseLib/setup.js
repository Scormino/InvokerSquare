...
function UpdateTeamPanel( teamPanel )
{
	...
	for ( var i = teamPlayers.length; i < nNumPlayerSlots; ++i )
	{
		...
		//LEN
		if(playerSlot == null && i == teamPlayers.length){
			//continue
			playerSlot = $.CreatePanel( "Panel", teamPanel.FindChildInLayoutFile( "PlayerList" ), "" );
			playerSlot.AddClass( "player_slot" );
			playerSlot.SetAttributeInt( "player_slot", i );
			//break
		}else{
			if(playerSlot != null && i >= teamPlayers.length+1){
				playerSlot.DeleteAsync(0)
			}
		}
...
{
	//$.Msg("function OnPlayerSelectedTeam( nPlayerId, nTeamId, bSuccess )")
	...
	//Setup_RefreshTeamsInfo()	//Вызова функции выше хватает сполна (это тут будет лишним)
}
...
{
	//$.Msg("function CheckForHostPrivileges()")
	var playerInfo = Game.GetLocalPlayerInfo();
	if ( !playerInfo )
		return;
...
//LEN
function Setup_RefreshTeamsInfo(){
	/*var data = {}
	var allTeamIDs = Game.GetAllTeamIDs()
	for ( var teamId of allTeamIDs ){
		data[teamId] = Game.GetPlayerIDsOnTeam( teamId )
	}*/


	if(RefreshAllMapSlots){
		RefreshAllMapSlots(null, 'hMapConstants') //[BaseLib/setup.js]
	}




	//Game.Setup.Setup_RefreshTeamsInfo.sNameFunc_fFunc


	
	//$.Msg("Team: "+teamId, "  players: ", Game.GetPlayerIDsOnTeam( teamId )
	//Game.db()
	//GameEvents.SendCustomGameEventToServer( "Setup_RefreshTeamsInfo", data )
}


//GameEvents.Subscribe( "Setup_RefreshTeamsInfo", MyFunc)
...
//SETUP
var sFileName = '[BaseLib/setup.js] '


var SetupRoot = $("#CustomGameSetup")
var CGSetup = $("#CGSetup")

var RefreshAllMapSlots
...
function RefreshCurrentGameMode(){
	var sGamemodeIndex = $("#DropDownGameMode").GetSelected().id
	//var sGamemodeName = $("#DropDownGameMode").GetSelected().text
	
	Game.Sync('addon', 'hGameSetup', {sGamemodeIndexSelected: sGamemodeIndex})
}

function SyncGamemodeSelected(){
	var SelectedNumber = parseInt(CustomNetTables.GetTableValue("addon", "hGameSetup").sGamemodeIndexSelected)
	$("#DropDownGameMode").SetSelected($("#DropDownGameMode").FindDropDownMenuChild(SelectedNumber))
	db(sFileName+'gamemode changed to '+$("#DropDownGameMode").FindDropDownMenuChild(SelectedNumber).text)		
}
...
//(function(){
	//убираем встроенный отступ у CustomUI
	var CUIContainer = Game.hudRoot.FindChildTraverse("CustomUIRoot").FindChildTraverse("CustomUIContainer_GameSetup").FindChildTraverse("CustomUIContainer")
	CUIContainer.style.marginLeft = "0px"
	
	//подгоняем чат загрузки под GameSetup
	var PanelChat = Game.LoadRoot.FindChildTraverse("LoadingScreenChat").GetParent()
	PanelChat.FindChildTraverse("ChatLinesContainer").style.height = "195px"
	PanelChat.FindChildTraverse("LoadingScreenChat").style.width = "350px"
	PanelChat.FindChildTraverse("LoadingScreenChat").style.marginLeft = "8px"
	PanelChat.FindChildTraverse("LoadingScreenChat").style.marginBottom = "8px"

	//инициализируем DropDown (ComboBox) для выбора Gamemode'а
	var DDGameMode = $("#DropDownGameMode")
	if(!DDGameMode.HasOption("0")){
		DDGameMode.RemoveAllOptions()
		var sGM_hGMInfo = CustomNetTables.GetTableValue("addon", "hGameSetup").sGM_hGMInfo
		
		for(var sGamemodeIndex in sGM_hGMInfo){
			var Label = $.CreatePanel( 'Label', DDGameMode, sGamemodeIndex)
			Label.text = sGM_hGMInfo[sGamemodeIndex].name

			if(sGM_hGMInfo[sGamemodeIndex].folder == ''){
				Label.style.backgroundColor = "black";
				Label.enabled = false;
				//db(sFileName, Label.text+'.folder == null !')
			}
			DDGameMode.AddOption(Label)
		}
		CustomNetTables.SubscribeNetTableListener("addon", SyncGamemodeSelected)
		SyncGamemodeSelected()
	}

	//Инициализируем изображение карты
	var sMap = Game.GetMapInfo().map_display_name
		//var sMapImagePath = "s2r://materials/overviews/" + sMap + ".tga"
	var sMapImagePath = "file://{images}/maps/" + sMap + ".tga"
	$("#MapImage").SetImage(sMapImagePath)

	//Загружаем данные server'а о текущей карте (а что он считает по этому поводу?)
	var hMapConstants = CustomNetTables.GetTableValue("addon", "hMapConstants")
	
	if(hMapConstants != undefined){
		for(var nTower in hMapConstants.nTower_hTower){
			var hTower = hMapConstants.nTower_hTower[nTower]

			var MapSlot = $.CreatePanel('Panel', $('#MapImage'), 'MapSlot_'+nTower)
			MapSlot.BLoadLayoutSnippet('MapSlot')
			if(hTower.bAllow){	
				//$.Msg("ADD CLASS='AllowMapSlot' by nTower = ", nTower)
				MapSlot.AddClass('AllowMapSlot')
			}else{
				//Если не разрешено садится в этот слот
				//$.Msg("ADD CLASS='NoAllowMapSlot' by nTower = ", nTower)
				MapSlot.AddClass('NoAllowMapSlot')

				//добавим tooltip:
				var ApplyTooltip = (
					function(MapSlot){
						return function(){
							$.DispatchEvent ( "DOTAShowTextTooltip" , MapSlot,  "HelpNoAllowSlot"); 
						}
					}
				)
				var DeleteTooltip = (
					function(MapSlot){
						return function(){
							$.DispatchEvent ("DOTAHideTextTooltip" , MapSlot); 
						}
					}
				)
				//Срабатывает при наведении мышкой
				MapSlot.SetPanelEvent("onmouseover", ApplyTooltip(MapSlot))

				//срабатывает, если убрать мушку за пределы панели
				MapSlot.SetPanelEvent("onmouseout", DeleteTooltip(MapSlot))
			}
			
			var xProc = hTower.hMiniMapPos.xProc
			var yProc = hTower.hMiniMapPos.yProc
			//$.Msg("MapSlot = ", MapSlot)
			MapSlot.style.marginTop = parseInt(yProc) + '%' 	// + ' - ' + MapSlot.height
			MapSlot.style.marginLeft = parseInt(xProc) + '%'	// + ' - ' + MapSlot.width

			var LeftClickEvent = (function(nLocalTower){
				return function(){
					$.Msg('CLICK Map slot = ', nLocalTower)
					var nLocalPlayer = Players.GetLocalPlayer()	//Определяем игрока, нажавшего на кнопку
					var hMapConstants = CustomNetTables.GetTableValue("addon", "hMapConstants")
					if(
						Players.GetTeam(nLocalPlayer) == DOTATeam_t.DOTA_TEAM_NOTEAM	//Если игрок не выбрал команду
						|| hMapConstants.nTower_hTower[nLocalTower].nPlayer != -1	//или если место, куда он переходит = занято другим игроком
						|| !hMapConstants.nTower_hTower[nLocalTower].bAllow	//или если слот запрещён
					){
						Game.EmitSound( "ui_team_select_pick_team_failed" );
						return	//То ничего не делаем
					}
					//Сначала удалим локального игрока из другого MapSlot'а (если он занят "нами")
					for(var nTower in hMapConstants.nTower_hTower){
						if(hMapConstants.nTower_hTower[nTower].nPlayer == nLocalPlayer){
							hMapConstants.nTower_hTower[nTower].nPlayer = -1	//очищаем слот от игрока
							break	//локальный игрок может быть только в 1 слоте, поэтому дальнейший перебор не имеет смысла
						}
					}
					hMapConstants.nTower_hTower[nLocalTower].nPlayer = nLocalPlayer	//присваиваем нового игрока
					Game.Sync('addon', 'hMapConstants', {nTower_hTower: hMapConstants.nTower_hTower})	//отправляем на синхронизацию
					Game.EmitSound( "ui_team_select_pick_team" );
				}
			})
			MapSlot.SetPanelEvent(("onactivate"), LeftClickEvent(nTower))	//Отлавливаем левый клик мышки
		}
		//MainKey тут не понадобится, этот аргумет передаётся в связи с CustomNetTables.SubscribeNetTableListener() (ниже)
		RefreshAllMapSlots = function(MainKey, TableKey, DATA){	//[custom_game/team_select.js]
			if(TableKey != 'hMapConstants'){return}	//Если изменение не касается обновления MapSlot'ов
			
			var hMapConstants = CustomNetTables.GetTableValue("addon", "hMapConstants")
			if(hMapConstants != undefined){
				var DeleteAvatarSlot = function(MapSlot, hTower){
					var MapSlotImage = MapSlot.FindChildTraverse('MapSlotImage')
					if(MapSlotImage){
						MapSlotImage.DeleteAsync(0)
					}
					MapSlot.style.boxShadow = 'black -2px -2px 5px 5px'
				}
				var CreateAvatarSlot = function(MapSlot, nCurrentTower, hTower){
					if(MapSlot.FindChildTraverse('MapSlotImage')){
						//Delete
						if(Players.GetTeam(hTower.nPlayer) == DOTATeam_t.DOTA_TEAM_NOTEAM){
							hMapConstants.nTower_hTower[nCurrentTower].nPlayer = -1
							Game.Sync('addon', 'hMapConstants', {nTower_hTower: hMapConstants.nTower_hTower})
							DeleteAvatarSlot(MapSlot, hTower)
						}
					}else{
						//Create
						//var MapSlotImage = $.CreatePanel("DOTAAvatarImage", MapSlot, 'MapSlotImage')
						MapSlot.BLoadLayoutSnippet('MapSlotImage')
						MapSlot.FindChildTraverse('MapSlotImage').steamid = Game.GetPlayerInfo(parseInt(hTower.nPlayer)).player_steamid
					}
					var sColor = GameUI.CustomUIConfig().team_colors[Players.GetTeam(hTower.nPlayer)]
					if(sColor){
						MapSlot.style.boxShadow = sColor+' -2px -2px 6px 6px'
					}else{
						MapSlot.style.boxShadow = 'black -2px -2px 5px 5px'
					}
				}
				...
				for(var nTower in hMapConstants.nTower_hTower){
					var hTower = hMapConstants.nTower_hTower[nTower]
					var MapSlot = $('#MapImage').FindChildTraverse('MapSlot_' + nTower)
					...
					if(hTower.nPlayer == -1){
						...
						//Игрок
						CreateAvatarSlot(MapSlot, nTower, hTower)
					}
				}
		...
		RefreshAllMapSlots(null, 'hMapConstants')
		CustomNetTables.SubscribeNetTableListener("addon", RefreshAllMapSlots)
	}
