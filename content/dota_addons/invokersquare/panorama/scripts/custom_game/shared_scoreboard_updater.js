...
function _ScoreboardUpdater_UpdatePlayerPanel( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
{
	...
	if ( playerInfo )
	{
		...
		var playerPortrait = playerPanel.FindChildInLayoutFile( "AvatarIcon" );
		if ( playerPortrait )
		{
		    playerPortrait.steamid = playerInfo.player_steamid
		    playerPortrait.style.width = "36px"
				playerPortrait.style.height = "36px" 
		...
		playerPortrait.GetParent().SetPanelEvent(("oncontextmenu"), 
			function(){
...
//		$.Msg( "UpdateTeamPanel.Create: ", teamPanelName, " = ", scoreboardConfig.teamXmlName );
	...
	//Len:
	_ScoreboardUpdater_SetTextSafe( teamPanel, "TeamTowerLabel", GetTowerCountByRealTeam(teamId))
	...
	if ( GameUI.CustomUIConfig().team_colors )
	{
		...
		{
			var gradientText = 'gradient( linear, 0% 0%, 100% 0%, from( #00000000 ), to( ' + teamColor + ' ) );';
//			$.Msg( gradientText );
...
//Lenivex:
var nRealTeam_nTowerCount = {}

function GetTowerCountByRealTeam(nTeam){
	if(nRealTeam_nTowerCount[nTeam] != null){
		return nRealTeam_nTowerCount[nTeam]
	}
	return 0
}

GameEvents.Subscribe("RefreshTeamTowersInfo", RefreshTeamTowersInfo)
function RefreshTeamTowersInfo(data){
	//$.Msg("[shared_scoreboard_updater.js RefreshTeamTowersInfo] data...")
	nRealTeam_nTowerCount = data.nRealTeam_nTowerCount
	//$.Msg("[shared_scoreboard_updater.js RefreshTeamTowersInfo] data complete")
	//$.Msg(data.nRealTeam_nTowerCount)
}
