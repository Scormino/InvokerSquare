...
function Setup_RefreshTeamsInfo(){
	...
	var hIDs = Game.GetAllPlayerIDs()	//Возвращает только реальных игроков
	for(var nPlayer in hIDs){
		db('nPlayer=', nPlayer)

	}

	$.Msg('Sha Wisawu')
	$.Msg(GameUI.RefreshAllMapSlots)
	if(GameUI.RefreshAllMapSlots){
		GameUI.RefreshAllMapSlots('hMapConstants') //[BaseLib/setup.js]
	}
